...
├─ 后台管理 CRUD
│  ├─ 简单 CRUD？
│  │  → Standard CRUD (QsListController + ListBuilder/FormBuilder)
│  │     - 单表操作
│  │     - 基础增删改查
│  │
│  └─ 多个相似模块？
...
// 最大长度
->addFormItem('title', 'text', '标题', '', '', '', 'maxlength="200"')

// 数字范围
->addFormItem('price', 'number', '价格', '', '', '', 'min="0" max="99999" step="0.01"')
...
// ❌ 避免 LIKE 前缀通配符
$where['title'] = ['like', '%keyword%'];  // 可以（但慢）
$where['title'] = ['like', 'keyword%'];   // 更好（可用索引）
...
// 清除所有同类缓存
QscmfCache::clearByTag('category_tag');
```
...
// 统一的错误响应格式
if ($error) {
    $this->error('错误描述');  // 返回标准错误
}

// 统一的成功响应格式
$this->success('成功描述', U('index'), $data);
```
