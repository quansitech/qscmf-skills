...
QSCMF API 控制器使用 `RestController` 作为基础类，提供统一的响应格式和认证机制。
...
```php
// 检查用户权限
protected function checkPermission($action): bool
{
...
public function gets(): Response
{
    ...
    if ($cached) {
        return new Response('成功', 1, json_decode($cached, true));
    }
    ...
    // 缓存结果（5 分钟）
    redis_set($cache_key, json_encode($result), 300);
