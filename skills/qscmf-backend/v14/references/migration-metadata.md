...
| @type 值 | 表单组件 | 表格组件 | 说明 | 迁移类型 |
|----------|----------|----------|------|----------|
| `image` / `picture` | image | image | 图片上传 | bigInteger |
...
| 元数据 | 生成的表单代码 |
|--------|---------------|
| `@title=名称;@require=true` | `$columns->text('field', '名称')->addRule(new Required())` |
| `@type=select;@list=status` | `$columns->select('field', '状态')->setValueEnum(DBCont::getStatusList())` |
| `@save=true` | 在表格列中添加 `->editable()` |

### 表格列生成

| 元数据 | 生成的表格代码 |
|--------|---------------|
| `@title=名称` | `$container->text('field', '名称')` |
| `@type=select;@list=status` | `$container->select('field', '状态')->setValueEnum(...)` |
...
```bash
# 从迁移文件解析
php scripts/parse_schema.php qs_xxx_cate

# 从数据库解析
php scripts/parse_schema.php qs_xxx_cate --from-db
```
