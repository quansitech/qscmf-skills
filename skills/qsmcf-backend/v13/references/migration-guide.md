...
> QSCMF v13 数据库迁移文件完整指南 - Laravel Schema Builder
...
$table->string('cover', 500)->nullable()->comment('封面图@title=封面@type=picture');
...
$table->boolean('is_enabled')->default(1)->comment('启用@title=启用@type=checkbox');
...
// 位置3: 控制器中调用
$builder->addFormItem('type', 'select', '类型', '', D('Product')->getTypeList());
```
...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('qs_product', function (Blueprint $table) {
            $table->bigIncrements('id');
            $table->string('product_name', 200)->comment('产品名称@title=产品名称@type=text');
            $table->bigInteger('cate_id')->default(0)->comment('分类ID@title=分类@type=select');
            $table->decimal('price', 10, 2)->default(0)->comment('价格@title=价格@type=number');
            $table->string('cover', 500)->nullable()->comment('封面图@title=封面图@type=picture');
            $table->text('summary')->nullable()->comment('简介@title=简介@type=textarea');
            $table->longText('content')->nullable()->comment('详情@title=详情@type=ueditor');
            $table->tinyInteger('status')->default(1)->comment('状态@title=状态@type=select@enum=status');
            $table->integer('sort')->default(0)->comment('排序@title=排序@type=number');
        ...
        // 表注释
        DB::unprepared("ALTER TABLE `qs_product` COMMENT = '@title=产品表;'");
    }
...
```php
Schema::table('qs_product', function (Blueprint $table) {
    $table->string('subtitle', 200)->nullable()->after('product_name')->comment('副标题');
...
```php
Schema::table('qs_product', function (Blueprint $table) {
    $table->string('product_name', 300)->change();
});
```
