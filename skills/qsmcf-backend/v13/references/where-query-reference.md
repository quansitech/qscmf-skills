...
从 PHP 8.2 开始，数据库查询结果会根据字段类型返回正确的类型：
...
// IN 查询类型问题
$id = D('User')->getField('id'); // PHP 8.2+ 返回 integer
if(empty($id)){
    $map['_string'] = '1=0';
}else{
    // ❌ PHP 8.2+ 会报错，因为 IN 期望字符串
    $map['model_id'] = ['IN', $id];  // 错误！
    // ✅ 需要转换为字符串
    $map['model_id'] = ['IN', (string)$id];  // 正确
}
```
